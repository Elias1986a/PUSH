    ...
    private func tokenize(text: String, addBos: Bool) -> [Token] {
        ...
        return Array(unsafeUninitializedCapacity: n_tokens) { buffer, initializedCount in
            initializedCount = Int(
                // PATCH: parse_special = true so ChatML control tokens
                // (<|im_start|>, <|im_end|>) tokenize as real special tokens.
                llama_tokenize(model, text, Int32(utf8Count), buffer.baseAddress, Int32(n_tokens), addBos, true)
            )
        }
    }
